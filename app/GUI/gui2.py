...
class MyFrame(wx.Frame):
    ...
    def radio_instru2(self, event):  # wxGlade: MyFrame.<event_handler>
        self.radio_instru(1,0,2,self.synth.instrument.output.setMul)
    def radio_instru1(self, event):  # wxGlade: MyFrame.<event_handler>
        self.radio_instru(0,0,2,self.synth.masterCtl[2].setMul)
    ...
    def refreshCtl(self):
        ...
        for i in range(5):
            ...
            for j in self.ctl:
                if(j in ctl):
                    self.instruInputBox[i].Append(str(j))
        self.combo_box_pan.Clear()
        for j in self.ctl:
            if(j in ctl):
                self.combo_box_pan.Append(str(j))
